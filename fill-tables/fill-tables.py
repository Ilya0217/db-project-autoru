...
def create_tables():
    ...
    with get_connection() as conn:
        with conn.cursor() as cursor:
            ...
            """)
            cursor.execute("""
                CREATE TABLE IF NOT EXISTS "CarBrandsAndModels" (
...
def fill_car_brands_and_models():
    """Заполняет таблицу CarBrandsAndModels случайными данными."""
    # Списки марок и моделей автомобилей
    brands = [
    ...
    for brand in brands:
        # Выбираем случайную модель для каждой марки
        model_name = random.choice(models[brands.index(brand)])
        values_list.append((brand, model_name))

    try:
        with get_connection() as conn:
            with conn.cursor() as cursor:
                insert_query = sql.SQL('INSERT INTO "CarBrandsAndModels"(BrandName, ModelName) VALUES (%s, %s)')
                cursor.executemany(insert_query, values_list)
                conn.commit()
                print("Таблица 'CarBrandsAndModels' успешно заполнена.")
    except Exception as e:
        print(f"Ошибка при заполнении таблицы 'CarBrandsAndModels': {e}")
...
def fill_announcements():
    ...
    for _ in range(ANNOUNCEMENTS_COUNT):  # Количество объявлений
        ...
        values_list.append((user_id, make, model_name, year, mileage, price, description, condition, status))
    try:
        with get_connection() as conn:
            with conn.cursor() as cursor:
                insert_query = sql.SQL('INSERT INTO "Announcement"(UserID, Make, Model, Year, Mileage, Price, Description, Condition, Status) VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s)')                
                cursor.executemany(insert_query ,values_list)
                conn.commit()
...
def fill_transactions():
     ...
     for _ in range(TRANSACTIONS_COUNT):  # Количество транзакций
         ...
         created_at = fake.date_time_this_year()  # Генерируем случайное время в этом году

         values_list.append((price ,buyer_id ,seller_id ,announcement_id, created_at))

     # Вставка данных в таблицу Transactions
     try:
         with get_connection() as conn:
             with conn.cursor() as cursor:
                 insert_query= sql.SQL('INSERT INTO "Transactions"(Price ,BuyerID ,SellerID ,AnnouncementID, CreatedAt) VALUES (%s ,%s ,%s,%s, %s)')
                 cursor.executemany(insert_query ,values_list)
                 conn.commit()
...
create_tables()
fill_car_brands_and_models()  # Заполнение таблицы марок и моделей автомобилей
